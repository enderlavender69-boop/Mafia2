...
async function collectBounty(targetId, collectorId, addCopper) {
  ...
  const credited = await addCopper(collectorId, bounty.total_amount);
  if (!credited) {
    // Previously the bounty row was deleted unconditionally right after this
    // call — if the credit failed, the collector got nothing AND the bounty
    // (and everyone's pooled money in it) was destroyed with no way to
    // retry. Leave the bounty intact so collection can be attempted again.
    console.error(`[BOUNTY COLLECT PAYOUT FAILED] collector=${collectorId} target=${targetId} amount=${bounty.total_amount} — bounty left in place for retry`);
    return { collected: 0, failed: true };
  }
