...
function formatWallet(wallet) {
  // Every earn/spend command in the bot builds its reply with
  // `eco.formatWallet(await eco.addCopper(...))`. If the credit failed,
  // addCopper resolves to null — previously this would then throw
  // (`Cannot read properties of null`) instead of showing a sensible
...
function walletToCopper(wallet) {
  // Approximate (Number) total — fine for comparisons/sorting/display, but
  // NOT for anything that writes a balance back to the DB (see
  // walletToCopperExact below). Uses Number(...) rather than raw `+` because
  // wallet.copper may now be a numeric string (see getWallet/fromCopperExact).
  return Math.floor(
    Number(wallet.copper  || 0) +
    Number(wallet.silver  || 0) * 100 +
    Number(wallet.gold    || 0) * 10000 +
    Number(wallet.stellar || 0) * 1000000
  );
}
...
// walletToCopper()/fromCopper() above go through a plain JS `Number`, which is
// a 64-bit float — it can only represent integers *exactly* up to
// Number.MAX_SAFE_INTEGER (~9,007t). Whale balances in this economy blow well
// past that (multiple wallets are already in the multi-million-trillion
// range), so every read-add-write through Number was silently rounding: small
// earnings (rob payouts, share-sale profit, job cash — anything under roughly
// a few thousand relative to the current balance) could round away to
// *nothing*, and larger ones landed a few hundred/thousand off. That's the
// root cause of "money doesn't get added" for anyone with a big balance —
// separate from (and in addition to) the write-failure cases the retry/
// refund logic below handles. A retried write of a rounded-away delta still
// writes back the same (wrong) total, since the rounding happens in JS before
// Supabase ever sees the request.
//
// Fix: any function that actually MUTATES a stored balance (adds/subtracts
// and writes back to Supabase) does the arithmetic in BigInt, which has no
// size ceiling, then serializes the result as a numeric STRING before saving
// (JSON — and therefore supabase-js — can't carry a raw BigInt, and a string
// avoids yet another float64 round-trip on the way into Postgres). Every
// other caller (comparisons, leaderboard sorting, display via fmt()) keeps
// using the plain walletToCopper()/Number path above — that's fine, since an
// imprecision of a few thousand Cash is invisible once formatted to 1 decimal
// place of "trillions," and it doesn't affect the actual transfer amount.
...
// Exact current balance as a BigInt, folding any legacy silver/gold/stellar
// in the same way walletToCopper() does, just without the float64 detour.
function walletToCopperExact(wallet) {
  return (
    toBigIntSafe(wallet.copper) +
    toBigIntSafe(wallet.silver) * 100n +
    toBigIntSafe(wallet.gold) * 10000n +
    toBigIntSafe(wallet.stellar) * 1000000n
  );
}

// Serializes an exact BigInt total back into wallet fields. Stored as a
// string so it survives JSON/Supabase without another lossy float64 pass.
function fromCopperExact(copperBigInt) {
...
async function getWallet(userId) {
  ...
  // next transaction. IMPORTANT: this uses walletToCopperExact (BigInt), not
  // the approximate walletToCopper Number — this is the value addCopper/
  // deductCopper/etc. build on, so it has to be exact or every mutation after
  // it inherits the rounding. Stored as a string so nothing downstream
  // accidentally round-trips it through a float64 again.
  const w = data[0];
  return { ...w, copper: walletToCopperExact(w).toString(), silver: 0, gold: 0, stellar: 0 };
}
...
async function saveWallet(wallet) {
  ...
  if (error) {
    console.error("[SAVE WALLET]", error.message);
    // Previously this only logged and returned normally, so callers like
    // addCopper()/deductCopper() would report success (and return the
    // in-memory wallet) even though the write never actually landed in the
    // database — money could silently vanish on large transactions (e.g. a
    // big stock sale) that tripped a DB-level failure such as numeric
    // overflow. Throwing here lets addCopper/deductCopper's existing
    // try/catch correctly return null so callers can detect and handle it.
    throw new Error(error.message);
  }
}

// Most real-world saveWallet failures are transient (a network blip or a
// momentary Supabase hiccup), not a genuine logic error. Every money-moving
// function in this file (addCopper, deductCopper, addDebt, payDebt,
// claimDaily) funnels through here, so retrying a handful of times here
// quietly fixes the vast majority of "money didn't arrive" reports across the
// entire bot without needing to touch every call site individually.
// It's safe to retry: saveWallet only resolves normally on a confirmed
// successful write, and only throws when Supabase reports the write did NOT
// apply, so a retry re-reads the current wallet and re-applies the same
// delta rather than double-applying anything.
async function withWalletRetry(fn, attempts = 3, delayMs = 200) {
  let lastErr;
...
async function payDebt(userId, amount) {
  try {
    ...
    // Deduct from balance using the EXACT total (BigInt), not the approximate
    // walletToCopper Number — same fix as addCopper/deductCopper above, so a
    // debt payment on a whale balance doesn't get silently short-changed.
    // Debt itself stays a plain Number on purpose — loans are capped to small
    // multiples of a daily payout, nowhere near the whale-wallet range, and a
    // lot of index.js does strict `=== 0` checks on `.debt` that a
    // stringified value would silently break.
    const totalCopper = walletToCopperExact(w);
    const payBig = toBigIntSafe(pay);
    if (totalCopper < payBig) return null;
    const newW = { ...w, debt: currentDebt - pay, ...fromCopperExact(totalCopper - payBig) };
    await saveWallet(newW);
    return newW;
...
// A retry-hardened, non-throwing wrapper around saveWallet() for the handful
// of call sites that need to write an arbitrary wallet shape directly (e.g.
// admin balance-set, loan payoff clearing `debt`) rather than a simple
// add/deduct — cases addCopper/deductCopper don't cover. Returns true/false
// instead of throwing, and gets the same retry treatment as addCopper so
// these aren't left worse-protected than the rest of the money-moving code.
async function saveWalletSafe(wallet) {
  try {
    await withWalletRetry(() => saveWallet(wallet));
    return true;
  } catch (e) {
    console.error(`[SAVE WALLET SAFE] user=${wallet?.user_id}`, e.message);
    return false;
  }
}
...
async function addCopper(userId, copperAmount) {
  return withUserLock(userId, async () => {
    try {
      return await withWalletRetry(async () => {
        const w = await getWallet(userId);
        const delta = toBigIntSafe(copperAmount);
        const total = walletToCopperExact(w) + delta;
        const earnedDelta = delta > 0n ? delta : 0n;
        const newW = {
          ...w,
          ...fromCopperExact(total),
          total_earned: (toBigIntSafe(w.total_earned) + earnedDelta).toString(),
        };
        await saveWallet(newW);
        return newW;
      });
    } catch (e) { console.error(`[ADD COPPER] user=${userId} amount=${copperAmount}`, e.message); return null; }
  });
}

// For the "already took money from someone, now paying it out to someone
// else" pattern (gambling payouts after the bet was deducted, robbery/heist
// cuts after the vault was already debited, etc). Tries to pay `payoutUserId`
// `payoutAmount`; if that fails even after addCopper's internal retries, it
// makes a best-effort refund of `refundAmount` to `refundUserId` so the
// original deduction isn't a total loss, and reports back what actually
// happened so the caller can tell the user (or log it for a manual fix)
// instead of silently pretending the payout worked.
async function payoutOrRefund(payoutUserId, payoutAmount, refundUserId, refundAmount) {
  if (payoutAmount <= 0) return { paid: true, refunded: false };
  const paidWallet = await addCopper(payoutUserId, payoutAmount);
  if (paidWallet) return { paid: true, refunded: false };
  console.error(`[PAYOUT FAILED] recipient=${payoutUserId} amount=${payoutAmount} — attempting refund of ${refundAmount} to ${refundUserId}`);
  if (!refundAmount || refundAmount <= 0) return { paid: false, refunded: false };
  const refunded = await addCopper(refundUserId, refundAmount);
  if (!refunded) console.error(`[REFUND ALSO FAILED] user=${refundUserId} amount=${refundAmount} — needs manual correction`);
  return { paid: false, refunded: !!refunded };
}
  ...
  return withUserLock(userId, async () => {
    try {
      return await withWalletRetry(async () => {
        const w = await getWallet(userId);
        const delta = toBigIntSafe(copperAmount);
        const total = walletToCopperExact(w);
        if (total < delta) return null;
        const newW = { ...w, ...fromCopperExact(total - delta) };
        await saveWallet(newW);
        return newW;
      });
    } catch (e) { console.error(`[DEDUCT COPPER] user=${userId} amount=${copperAmount}`, e.message); return null; }
  });
}
...
// Mr.EnderLavender's cut is effectively bottomless.
const DAILY_REWARDS = {
  streetrat:   100,
  associate:   500,
  soldier:     1000,
...
// 1000000000 -> "1b", up through "qd" (quadrillion), "qt" (quintillion), and
// beyond (see AMOUNT_SUFFIXES above — same table parseBet() reads suffixes
// from, so what you type and what you see always agree). One decimal place,
