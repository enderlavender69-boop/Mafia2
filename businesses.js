...
const BUSINESS_TYPES = {
  laundromat: {
    ...
    tiers: [
      { cost: 50_000,    income: 4_500,    upkeep: 500,    capacity: 60_000 },
      { cost: 150_000,   income: 11_500,   upkeep: 1_250,  capacity: 150_000 },
      { cost: 400_000,   income: 28_000,   upkeep: 3_000,  capacity: 400_000 },
      { cost: 1_000_000, income: 67_500,   upkeep: 7_500,  capacity: 1_000_000 },
      { cost: 2_500_000, income: 170_000,  upkeep: 17_500, capacity: 2_500_000 },
    ],
  },
  nightclub: {
    label: "🎷 Nightclub", flavor: ["Back Alley Bar", "The Velvet Room", "Club Onyx", "The Silver Fox", "The Golden Peacock"],
    tiers: [
      { cost: 150_000,   income: 11_500,   upkeep: 1_500,  capacity: 150_000 },
      { cost: 400_000,   income: 31_000,   upkeep: 3_750,  capacity: 400_000 },
      { cost: 1_000_000, income: 79_000,   upkeep: 8_750,  capacity: 1_000_000 },
      { cost: 2_500_000, income: 197_500,  upkeep: 21_250, capacity: 2_500_000 },
      { cost: 6_000_000, income: 478_000,  upkeep: 50_000, capacity: 6_000_000 },
    ],
  },
  shipping: {
    ...
    tiers: [
      { cost: 300_000,   income: 23_000,   upkeep: 3_000,  capacity: 300_000 },
      { cost: 800_000,   income: 57_000,   upkeep: 7_000,  capacity: 800_000 },
      { cost: 2_000_000, income: 148_000,  upkeep: 16_250, capacity: 2_000_000 },
      { cost: 5_000_000, income: 368_000,  upkeep: 40_000, capacity: 5_000_000 },
      { cost: 12_000_000, income: 900_000, upkeep: 100_000, capacity: 12_000_000 },
    ],
  },
  casino: {
    label: "🎰 Casino", flavor: ["The Backroom Table", "Lucky Sevens", "The High Roller", "Diamond Point Casino", "The Emerald Palace"],
    tiers: [
      { cost: 800_000,    income: 57_000,    upkeep: 8_750,   capacity: 800_000 },
      { cost: 2_000_000,  income: 148_000,   upkeep: 20_000,  capacity: 2_000_000 },
      { cost: 5_000_000,  income: 368_000,   upkeep: 47_500,  capacity: 5_000_000 },
      { cost: 12_000_000, income: 900_000,   upkeep: 112_500, capacity: 12_000_000 },
      { cost: 30_000_000, income: 2_250_000, upkeep: 275_000, capacity: 30_000_000 },
    ],
  },
};
...
async function collectBusiness(userId, type, addCopper) {
  ...
  const credited = await addCopper(userId, net);
  if (!credited) {
    // Don't clear pending income if the player never actually got paid —
    // previously this cleared unconditionally, so a failed credit here meant
    // the income was gone with nothing to show for it.
    console.error(`[BUSINESS COLLECT PAYOUT FAILED] user=${userId} amount=${net} biz=${biz.id} — pending income left intact for retry`);
    return { success: false, reason: "Something went wrong collecting — your pending income is untouched. Try again." };
  }
...
async function raidBusiness(raiderId, ownerId, type, addCopper) {
  ...
  const credited = await addCopper(raiderId, stolen);
  if (!credited) {
    // The business's pending income was already reduced — if the raider
    // never actually received it, put it back rather than let it vanish.
    console.error(`[BUSINESS RAID PAYOUT FAILED] raider=${raiderId} amount=${stolen} biz=${biz.id} — restoring pending income`);
    await supabase.from("businesses").update({ pending: biz.pending }).eq("id", biz.id).catch(e => console.error("[RAID REFUND ALSO FAILED]", e.message));
    return { success: false, reason: "Something went wrong collecting the raid — try again." };
  }
