...
const FIRM_RULES = [
  ...
  "Mr.EnderLavender may sanction, crash, or dissolve your firm at any time for violations.",
...
async function confirmFirmCreation(userId) {
  ...
  if (existing && !existing.dissolved) {
    const refunded = await eco.addCopper(userId, FIRM_CREATION_FEE);
    pendingCreations.delete(userId);
    if (!refunded) {
      console.error(`[FIRM CREATE REFUND FAILED] user=${userId} amount=${FIRM_CREATION_FEE} — needs manual correction`);
      return `🔫 Ticker **${pending.ticker}** was just taken, and the refund failed to process. Contact an admin to get your ${formatCopper(FIRM_CREATION_FEE)} back.`;
    }
...
async function buyFirmShares(userId, ticker, amount) {
  ...
  if (taxAmount > 0) {
    const taxCredited = await eco.addCopper(MASTER_ID, taxAmount);
    if (!taxCredited) console.error(`[FIRM BUY TAX FAILED] amount=${taxAmount} firm=${firm.ticker} — buyer already charged, tax not collected`);
  }
...
async function sellFirmShares(userId, ticker, amount) {
  ...
  // Credit the seller FIRST and confirm it actually landed before touching
  // holdings, the firm's treasury, or the market price — same class of bug as
  // the stock-market sellStock() fix: previously all of that mutated
  // up-front and the payout result was discarded, so a failed credit meant
  // the seller lost their shares for nothing (and the price still moved on a
  // sale that never actually completed).
  const credited = await eco.addCopper(userId, payout);
  if (!credited) {
...
async function payDividends(userId, ticker, totalAmountCu) {
  ...
  for (const [hId, shares] of Object.entries(firm.holdings)) {
    ...
    if (payout > 0) {
      const credited = await eco.addCopper(hId, payout);
      if (credited) {
        actuallyPaid += payout;
        results.push({ hId, shares, payout });
      } else {
        console.error(`[DIVIDEND PAYOUT FAILED] holder=${hId} amount=${payout} firm=${firm.ticker}`);
        failed.push({ hId, shares, payout });
      }
    }
  }

  // Only remove from the treasury what actually reached a holder — previously
  // the full totalAmountCu was debited up front regardless of individual
  // payout outcomes, so any failed payout meant that share of the money
  // vanished from the treasury without ever reaching the holder.
  firm.treasury -= actuallyPaid;
  firm.total_dividends_paid += actuallyPaid;

  await dbSaveFirm(firm);

  const lines = results.map(r => `  <@${r.hId}> — ${r.shares} shares → **${formatCopper(r.payout)}**`).join("\n");
  const failedLines = failed.length > 0
    ? `\n⚠️ **Failed, left in treasury for retry:** ${failed.map(f => `<@${f.hId}>`).join(", ")}`
    : "";
  return (
    `💸 **DIVIDENDS PAID — ${firm.name}** (\`${firm.ticker}\`)\n` +
    `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
    `Total distributed: **${formatCopper(actuallyPaid)}**\n` +
...
async function donDeleteFirm(ticker, reason) {
  ...
  for (const [hId, shares] of Object.entries(firm.holdings)) {
    ...
    if (payout > 0) {
      ...
      if (credited) {
        ...
        // Previously totalRefunded incremented here regardless of success,
        // which both shortchanged the holder AND silently shrank the
        // "remainder to the Don" figure below to compensate — the money
        // vanished from the books entirely instead of going to the holder
        // OR the Don. Now a failed credit is excluded from totalRefunded and
        // called out explicitly so it can be manually corrected.
        console.error(`[FIRM DELETE REFUND FAILED] holder=${hId} amount=${payout} firm=${firm.ticker} — needs manual correction`);
        failedRefunds.push(`  <@${hId}> — ${shares} shares → **${formatCopper(payout)}** ⚠️ FAILED, needs manual credit`);
      }
    }
  }

  const remainder = firm.treasury - totalRefunded;
  if (remainder > 0) await eco.addCopper(MASTER_ID, remainder).catch(() => {});
  ...
  const failedBlock = failedRefunds.length
    ? `\n\n⚠️ **Failed refunds (need manual correction):**\n${failedRefunds.join("\n")}`
    : "";
...
async function donCrashFirmShares(ticker, percent, reason, channel) {
  ...
  if (shareholders.length && channel) {
    ...
    await channel.send(
      ...
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `${pings}\n\n` +
      `**${firm.name}** (\`${firm.ticker}\`) has been **CRASHED by ${percent}%** by order of Mr.EnderLavender.\n` +
...
async function donEscalateSanction(ticker, reason, channel) {
  ...
  if (channel && shareholders.length) {
    ...
    await channel.send(
      ...
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `${pings}\n\n` +
      `Mr.EnderLavender has **ESCALATED sanctions** on this firm.\n` +
