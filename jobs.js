...
async function doWork(userId, rankLevel, isDon) {
  ...
  const payLine = newW
    ...
    : `You ${job.verb} ${job.where}, but crediting your **💵 ${eco.fmt(pay)} Cash** failed. Try **Cosa balance** in a moment — if it's still missing, contact an admin.`;
  return (
...
async function doCrime(userId, rankLevel, isDon, deps = {}) {
  ...
  if (roll < 0.55) {
    ...
    const payLine = newW
      ...
      : `You ${pick(CRIME_SUCCESS)}, but crediting your **💵 ${eco.fmt(pay)} Cash** failed. Try **Cosa balance** in a moment — if it's still missing, contact an admin.`;
    return (
      `🔫 **Score!**\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      payLine + fastHandsLine
    );
  } else if (roll < 0.85) {
...
async function doScavenge(userId, rankLevel, isDon) {
  ...
  const payLine = newW
    ...
    : `You ${pick(SCAVENGE_FINDS)}, but crediting your **💵 ${eco.fmt(pay)} Cash** failed.${bonusLine} Try **Cosa balance** in a moment — if it's still missing, contact an admin.`;
  return (
    `🔦 **Scavenging**\n` +
    payLine +
    (fastHandsUsed ? `\n⚡ **Fast Hands** cut this cooldown in half!` : "")
...
async function doSmuggle(userId, rankLevel, isDon, deps = {}) {
  ...
  if (Math.random() < 0.48) {
    ...
    const payLine = newW
      ...
      : `You ${pick(SMUGGLE_ROUTES)}, but crediting your **💵 ${eco.fmt(pay)} Cash** failed. Try **Cosa balance** in a moment — if it's still missing, contact an admin.`;
    return (
      `🚢 **Shipment delivered.**\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      payLine + fastHandsLine
...
async function claimQuest(userId) {
  ...
  return (
    ...
    `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
    `Mr.EnderLavender digs into the Family stash and tosses you some loot:\n` +
