...
async function withdrawFromTreasury(userId, amount, addCopper) {
  ...
  if (addCopper) {
    const credited = await addCopper(userId, amount);
    if (!credited) {
      // The treasury was already debited — if the member never actually
      // received it, put it back rather than let it vanish.
      console.error(`[GANG WITHDRAW PAYOUT FAILED] user=${userId} amount=${amount} gang=${ug.gang.id} — returning to treasury`);
      await addToGangTreasury(ug.gang.id, amount).catch(e => console.error("[GANG WITHDRAW REFUND ALSO FAILED]", e.message));
      withdrawCooldowns.delete(ug.gang.id);
      return { success: false, reason: "Something went wrong crediting the withdrawal — the Cash was returned to the treasury. Try again." };
    }
  }
...
const pendingBribes = new Map(); // targetUserId -> { fromGangId, fromGangName, amount, offeredBy, createdAt, timeout }
...
async function offerBribe(actorId, targetUserId, amount, deductFromWallet, addCopper) {
  ...
  const timeout = setTimeout(async () => {
    ...
    if (pending && pending.offeredBy === actorId) {
      ...
      const refunded = await addCopper(actorId, pending.amount); // unanswered — refund in full
      if (!refunded) console.error(`[BRIBE TIMEOUT REFUND FAILED] user=${actorId} amount=${pending.amount} — needs manual correction`);
    }
  }, BRIBE_EXPIRY_MS);
...
async function acceptBribe(targetUserId, addCopper) {
  ...
  if (leaveError) {
    ...
    const refunded = await addCopper(offer.offeredBy, offer.amount); // refund — the move failed
    if (!refunded) console.error(`[BRIBE LEAVE REFUND ALSO FAILED] user=${offer.offeredBy} amount=${offer.amount} — needs manual correction`);
    return { success: false, reason: "Database error moving you over — the offering leader's Cash was refunded. " + leaveError.message };
  }
  ...
  if (joinError) {
    ...
    const refunded = await addCopper(offer.offeredBy, offer.amount); // refund — you didn't actually end up in their gang
    if (!refunded) console.error(`[BRIBE JOIN REFUND ALSO FAILED] user=${offer.offeredBy} amount=${offer.amount} — needs manual correction`);
    return { success: false, reason: "Database error joining your new gang — the offering leader's Cash was refunded. " + joinError.message };
  }

  const paid = await addCopper(targetUserId, offer.amount);
  if (!paid) {
    // The membership move already went through and can't be cleanly undone
    // at this point — flag it clearly so it can be manually corrected rather
    // than the bribed member simply never getting paid.
    console.error(`[BRIBE PAYOUT FAILED] user=${targetUserId} amount=${offer.amount} — gang move completed, payout needs manual correction`);
    return { success: true, amount: offer.amount, oldGangName, newGangName: offer.fromGangName, payoutFailed: true };
  }
...
async function declineBribe(targetUserId, addCopper) {
  ...
  clearTimeout(offer.timeout);
  pendingBribes.delete(targetUserId);
  const refunded = await addCopper(offer.offeredBy, offer.amount); // refund the offering leader in full
  if (!refunded) console.error(`[BRIBE DECLINE REFUND FAILED] user=${offer.offeredBy} amount=${offer.amount} — needs manual correction`);
  return { success: true, fromGangName: offer.fromGangName, amount: offer.amount, refundFailed: !refunded };
}
