...
const TRIVIA_QUESTIONS = [
  ...
  // ── Family/mob flavor ──
  { q: "In mob slang, what's a 'made man'?", a: "a full member of the family", choices: ["A new recruit", "A full member of the family", "A police informant", "A rival boss"] },
  { q: "What Italian term refers to a mafia code of silence?", a: "omerta", choices: ["Omerta", "Cosa Nostra", "Vendetta", "Padrino"] },
  { q: "What does 'Cosa Nostra' translate to in English?", a: "our thing", choices: ["Our Family", "Our Thing", "Our Business", "Our Honor"] },
  { q: "In The Godfather, what's the name of Vito Corleone's youngest son?", a: "michael", choices: ["Sonny", "Fredo", "Michael", "Tom"] },
  { q: "What city is widely considered the birthplace of the American Mafia?", a: "new york", choices: ["Chicago", "New York", "Las Vegas", "Boston"] },
  { q: "What's a 'consigliere' in mafia hierarchy?", a: "advisor", choices: ["Enforcer", "Advisor", "Bookkeeper", "Lookout"] },
  { q: "What Chicago gangster was finally imprisoned for tax evasion?", a: "al capone", choices: ["John Dillinger", "Al Capone", "Bugsy Siegel", "Lucky Luciano"] },
  // ── General knowledge ──
  { q: "What is the tallest mountain in the world?", a: "everest", choices: ["K2", "Everest", "Kilimanjaro", "Denali"] },
  { q: "What is the largest mammal on Earth?", a: "blue whale", choices: ["Elephant", "Blue Whale", "Giraffe", "Hippo"] },
  { q: "How many colors are in a rainbow?", a: "7", choices: ["5", "6", "7", "8"] },
  { q: "What is the capital of Italy?", a: "rome", choices: ["Milan", "Venice", "Rome", "Naples"] },
  { q: "What metal is liquid at room temperature?", a: "mercury", choices: ["Lead", "Mercury", "Iron", "Tin"] },
  { q: "What organ pumps blood through the body?", a: "heart", choices: ["Liver", "Heart", "Lungs", "Kidney"] },
  { q: "How many players are on a standard soccer team on the field?", a: "11", choices: ["9", "10", "11", "12"] },
  { q: "What is the capital of Canada?", a: "ottawa", choices: ["Toronto", "Vancouver", "Ottawa", "Montreal"] },
  { q: "What year did the Titanic sink?", a: "1912", choices: ["1910", "1912", "1914", "1916"] },
  { q: "What is the study of stars and space called?", a: "astronomy", choices: ["Astrology", "Astronomy", "Geology", "Biology"] },
  { q: "What is the freezing point of water in Celsius?", a: "0", choices: ["-10", "0", "10", "32"] },
  { q: "What country gifted the Statue of Liberty to the US?", a: "france", choices: ["France", "England", "Spain", "Italy"] },
  { q: "What is the main ingredient in guacamole?", a: "avocado", choices: ["Tomato", "Avocado", "Lime", "Onion"] },
  { q: "How many teeth does an adult human typically have?", a: "32", choices: ["28", "30", "32", "34"] },
  { q: "What is the currency of the United Kingdom?", a: "pound", choices: ["Euro", "Pound", "Franc", "Dollar"] },
  { q: "What planet is known as the Red Planet?", a: "mars", choices: ["Venus", "Mars", "Jupiter", "Saturn"] },
  { q: "What is the largest desert in the world?", a: "antarctic", choices: ["Sahara", "Gobi", "Antarctic", "Arabian"] },
  { q: "What instrument has 88 keys?", a: "piano", choices: ["Organ", "Piano", "Harpsichord", "Accordion"] },
];
...
async function endTriviaTournament(channelId, guild, tournament) {
  ...
  const prize = tournament.prizeCash;
  ...
  if (roll <= successChance) {
    ...
    if (hasAlarm) {
      ...
      for (const id of crewIds) {
        if (cut <= 0) continue;
        const credited = await eco.addCopper(id, cut);
        if (credited) {
          eco.markTainted(id, cut);
        } else {
          // The vault was already debited for this cut — if crediting the crew
          // member fails, put it back in the vault rather than let it vanish,
          // and flag it so the failure is visible instead of silent.
          console.error(`[BANK ROB PAYOUT FAILED] user=${id} cut=${cut} — returning to ${rob.targetId}'s vault`);
          await bank.deposit(rob.targetId, cut).catch(e => console.error("[BANK ROB REFUND ALSO FAILED]", e.message));
          failedCrew.push(id);
        }
      }
      const failedLine = failedCrew.length > 0
        ? `\n⚠️ ${failedCrew.map(id => `<@${id}>`).join(", ")} — payout failed and was returned to the vault. Try again or contact an admin.`
        : "";
      await channel.send(
        `💰 **VAULT CRACKED!**\n${crewMentions}\nNo alarm to stop them — the crew made off with **${bank.formatCopper(deducted)}** from **${targetName}'s** vault (**${bank.formatCopper(cut)}** each) before they even knew what hit them. 😈${failedLine}`
      ).catch(() => {});
    }
  } else {
    ...
    if (totalToVictim > 0) {
      const victimPaid = await eco.addCopper(rob.targetId, totalToVictim);
      if (!victimPaid) console.error(`[BANK ROB FINE PAYOUT FAILED] victim=${rob.targetId} amount=${totalToVictim} — collected from crew but never delivered, needs manual correction`);
    }
    if (totalToVig > 0) {
      const vigPaid = await eco.addCopper(MASTER_ID, totalToVig);
      if (!vigPaid) console.error(`[BANK ROB FINE PAYOUT FAILED] vig=${MASTER_ID} amount=${totalToVig} — collected from crew but never delivered, needs manual correction`);
    }
...
async function resolveVaultAlarm(token, channel, defused) {
  ...
  for (const id of alarm.crewIds) {
    ...
    if (credited) {
      ...
      console.error(`[VAULT ALARM PAYOUT FAILED] user=${id} cut=${cut} — returning to ${alarm.targetId}'s vault`);
      await bank.deposit(alarm.targetId, cut).catch(e => console.error("[VAULT ALARM REFUND ALSO FAILED]", e.message));
      failedCrew.push(id);
    }
  }
  const failedLine = failedCrew.length > 0
    ...
    : "";
  await channel.send(
    `💰 **VAULT CRACKED!**\n${crewMentions}\nNo alarm in time — the crew made off with **${bank.formatCopper(deducted)}** from **${targetName}'s** vault (**${bank.formatCopper(cut)}** each). 😈${failedLine}`
  ).catch(() => {});
...
async function executeHeist(channelId, guild) {
  ...
  if (roll < successChance) {
    ...
    // Each grabber already paid an entry fee to get here — if their payout
    // fails, at least refund the entry fee so they aren't simply out that
    // money for nothing, and flag it so it's visible rather than silent.
    const failedGrabbers = [];
    for (const uid of grabbedIds) {
      const credited = await eco.addCopper(uid, perPerson);
      if (!credited) {
        console.error(`[HEIST PAYOUT FAILED] user=${uid} amount=${perPerson} — refunding entry fee ${heist.entryFee}`);
        await eco.addCopper(uid, heist.entryFee).catch(e => console.error("[HEIST REFUND ALSO FAILED]", e.message));
        failedGrabbers.push(uid);
      }
    }
    await eco.deductCopper(MASTER_ID, heist.vaultCopper).catch(() => {});
    const failedLine = failedGrabbers.length > 0
      ? `\n⚠️ ${failedGrabbers.map(id => `<@${id}>`).join(", ")} — payout failed, entry fee refunded instead. Contact an admin if this happens again.`
      : "";
    await channel.send(
      `💰 **HEIST SUCCESSFUL!** 💰\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      ...
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `*The Family's vault has been hit. Mr.EnderLavender is NOT pleased. 😤*${failedLine}`
    ).catch(() => {});
  } else {
    await eco.addCopper(MASTER_ID, totalPot).catch(() => {});
    await channel.send(
...
// Per-target lock so two concurrent proposals to the SAME person can't race:
// without this, both could pass the "already has a pending proposal" check
// before either sets the map entry, and the second .set() would silently
// overwrite the first — losing that person's ring money with no refund (the
// first proposal's own 60s timeout would go on to delete whatever entry is
// CURRENTLY there, i.e. the second person's still-valid proposal, refunding
// the wrong person). Mirrors economy.js's withUserLock pattern.
const proposalLocks = new Map(); // targetId -> chained Promise
function withProposalLock(targetId, fn) {
  const prev = proposalLocks.get(targetId) || Promise.resolve();
  const run = prev.then(fn, fn);
  proposalLocks.set(targetId, run.catch(() => {}));
  return run;
}
...
async function proposeMarriage(proposerId, targetId, guild, channelId) {
  ...
  if (targetId === MASTER_ID) return "🚫 You dare propose to Mr.EnderLavender? Absolutely not.";
  ...
  return withProposalLock(targetId, async () => {
    // Re-check inside the lock — someone else's proposal to this same target
    // may have landed while we were awaiting the checks above.
    ...
    const timeout = setTimeout(async () => {
      const cur = pendingProposals.get(targetId);
      // Only clear/refund if THIS proposal is still the one on file — a
      // stale timeout must never delete or refund a different, newer
      // proposal that's since taken its place.
      if (cur && cur.proposerId === proposerId) {
...
async function acceptProposal(targetId, guild, channelId) {
  return withProposalLock(targetId, async () => {
    ...
    const { error } = await supabase.from("marriages").upsert([
    ...
    if (error) {
      console.error("[MARRY ACCEPT]", error.message);
      await eco.addCopper(proposal.proposerId, MARRIAGE_COST).catch(() => {});
      return "🔫 Something went wrong finalizing the marriage — ring refunded, try proposing again.";
    }
...
async function divorce(userId) {
  ...
  // Single request for both rows, not two separate awaited calls — the old
  // code could delete userId's row, then fail (network blip) on the
  // partner's row, leaving them stuck "married" to someone who'd already
  // moved on with no way back in short of a manual DB fix. One `.in()` call
  // either removes both rows or neither.
  const { error } = await supabase.from("marriages").delete().in("user_id", [userId, marriage.partnerId]);
  if (error) {
    console.error("[DIVORCE]", error.message);
    await eco.addCopper(userId, DIVORCE_COST).catch(() => {});
    return "🔫 Something went wrong processing the divorce — fee refunded, try again.";
  }
...
const SHOP_ITEMS = {
  ...
  kings_call: {
    ...
    desc: "Summons Mr.EnderLavender to intervene in the market. He decides which stock and whether to pump or crash. 24h server cooldown. No refunds.",
...
// ── Admin: wipe a user's entire shop inventory (Mr.EnderLavender only, enforced by caller) ─
