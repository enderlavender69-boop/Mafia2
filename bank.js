...
// Same fix as economy.js: `account.balance` was a plain JS Number, which is
// only exact up to ~9,007t (Number.MAX_SAFE_INTEGER). Whale bank balances
// blow well past that, so every deposit/withdraw/interest tick was silently
// rounding — small amounts could vanish entirely, larger ones landed a bit
// off. Fixed the same way: do the actual add/subtract in BigInt (no size
// ceiling) and store the result as an exact digit string.
...
async function processBank(account, masterId, addToTreasury) {
  ...
  const balanceBig = toBigIntSafe(account.balance);
  if (balanceBig <= 0n) {
...
async function deposit(userId, copperAmount) {
  ...
  const tier = VAULT_TIERS[account.vault_tier] || VAULT_TIERS.shoebox;
  const currentBig = toBigIntSafe(account.balance);
  const depositBig = toBigIntSafe(copperAmount);
  // maxStorage is a plain Number (Infinity for the Don's Vault — see
...
async function withdraw(userId, copperAmount) {
  ...
  const account = await getBankAccount(userId);
  const currentBig = toBigIntSafe(account.balance);
  const withdrawBig = toBigIntSafe(copperAmount);
  if (withdrawBig > currentBig) return { success: false, reason: "Insufficient bank balance." };
  account.balance = (currentBig - withdrawBig).toString();
  await saveBankAccount(account);
  return { success: true, account };
}
...
async function deductFromBank(userId, amount) {
  ...
  if (currentBig < amountBig) {
    // Deduct what we can
    account.balance = "0";
    await saveBankAccount(account);
    return Number(currentBig); // approximate, only used for display/logging by callers
  }
  account.balance = (currentBig - amountBig).toString();
  await saveBankAccount(account);
  return amount;
}
